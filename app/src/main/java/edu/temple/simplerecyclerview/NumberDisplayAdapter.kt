...
class NumberDisplayAdapter (private val numbers: Array<Int>, private val callBackLambda: (Int) -> Unit) : RecyclerView.Adapter<NumberDisplayAdapter.NumberViewHolder>() {
    //callBackLambda of this signature: (Int) -> Unit
    ...
    inner class NumberViewHolder (layout: View) : RecyclerView.ViewHolder (layout) {
        ...
        val textView = layout.findViewById<TextView>(R.id.textView).apply {
            setOnClickListener {
                callBackLambda(numbers[adapterPosition])
            }
        }
